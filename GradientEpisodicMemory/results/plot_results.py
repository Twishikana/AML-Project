...
for dataset in datasets:
    ...
    for model in models:
        ...
        if len(matches):
            data = torch.load(
                matches[0], map_location=lambda storage, loc: storage)
            acc, bwt, fwt = data[3][:]
...
for dataset in datasets:
    ...
    plt.xticks([.5, 3.5, 6.5], ['ACC', 'BWT', 'FWT'], fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('barplot_%s.png' % dataset, bbox_inches='tight')
    # plt.show()
...
for dataset in datasets:
    ...
    plt.title(names_datasets[dataset], fontsize=16)
    plt.tight_layout()
    plt.savefig('evoplot_%s.png' % dataset, bbox_inches='tight')
    # plt.show()
